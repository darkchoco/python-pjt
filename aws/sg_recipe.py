#!/usr/bin/env python
"""
Recipe for creating and updating security groups pragmatically.
...
def modify_sg(c, group, rule, do_authorize=False, do_revoke=False):
    src_group = None
    if rule.src_group_name:
        src_group = c.get_all_security_groups([rule.src_group_name, ])[0]

    if do_authorize and not do_revoke:
        ...
        group.authorize(ip_protocol=rule.ip_protocol,
                        ...
                        src_group=src_group)
    elif not do_authorize and do_revoke:
...
def authorize(c, group, rule):
    ...
    return modify_sg(c, group, rule, do_authorize=True)


def revoke(c, group, rule):
    """Revoke `rule` on `group`."""
    return modify_sg(c, group, rule, do_revoke=True)
...
if __name__ == "__main__":
    create_security_groups()  # !/usr/bin/env python
